 ...
 */
function Graph3d(container, data, options, whichX) {
...
Graph3d.prototype.setOptions = function (options) {
  ...
  if (options !== undefined) {
    ...
    if (options.innerYValueLabels !== undefined) { // DEC19
      this.innerYValueLabels = options.innerYValueLabels;
    }

    if (options.outerYValueLabels !== undefined) { // DEC19
      this.outerYValueLabels = options.outerYValueLabels;
    }

    if (options.farOuterYValueLabels !== undefined) { // DEC19
  ...
  if (this.style === Graph3d.STYLE.SURFACE) {
    ...
    for (i = 0; i < this.dataPoints.length; i++) {
      ...
      if (point !== undefined && top !== undefined) {
        ...
        if (this.whichXToHighlight !== undefined && point.bottom.x == this.whichXToHighlight) { 
          ctx.lineWidth = this._getStrokeWidth(point) * 4;
          ctx.strokeStyle = this._hsv2rgb(240, 2, 2);
        } else {
          ctx.lineWidth = this._getStrokeWidth(point) * 2;
          ctx.strokeStyle = this._hsv2rgb(h, 1, 1);
        }
...
Graph3d.prototype._redrawDataBar = function() {
  ...
  for (i = 0; i < this.dataPoints.length; i++) {
    ...
    var colorFn = function(c,n,i,d){for(i=3;i--;c[i]=d<0?0:d>255?255:d|0)d=c[i]+n;return c}
    ...
    if (this.whichXToHighlight !== undefined && point3d.x == this.whichXToHighlight) {
      var vals = ( colorFn(color.match(numberPattern).map(function(num){return parseInt(num);}), 110) )
